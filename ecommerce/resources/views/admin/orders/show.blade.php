...
<head>
    ...
    <style>
        ...
        .meta { color: #6b7280; font-size: .95rem; margin-bottom: 1.5rem; }
        ...
        .card h2 { font-size: 1rem; margin: 0 0 .75rem; border-bottom: 1px solid #e5e7eb; padding-bottom: .4rem; }

        table { width: 100%; border-collapse: collapse; }
        th, td { text-align: left; padding: .55rem .75rem; border-bottom: 1px solid #e5e7eb; font-size: .9rem; }
        ...
        .summary-total { font-weight: 700; border-top: 2px solid #111; margin-top: .25rem; padding-top: .4rem; }

        address { font-style: normal; line-height: 1.7; font-size: .9rem; }

        .badge {
            display: inline-block; padding: .2rem .55rem;
            border-radius: 20px; font-size: .8rem; font-weight: 600; text-transform: capitalize;
        }
        .badge-pending    { background: #fff3cd; color: #856404; }
        .badge-paid       { background: #d1e7dd; color: #0f5132; }
        .badge-processing { background: #cfe2ff; color: #084298; }
        .badge-shipped    { background: #e0cffc; color: #432874; }
        ...
        .alert-success { background: #d1e7dd; border: 1px solid #badbcc; color: #0f5132; padding: .75rem 1rem; border-radius: 4px; margin-bottom: 1rem; }
        .alert-error   { background: #f8d7da; border: 1px solid #f5c2c7; color: #842029; padding: .75rem 1rem; border-radius: 4px; margin-bottom: 1rem; }

        .status-form { margin-top: .75rem; display: flex; gap: .5rem; align-items: center; }
        .status-form select { padding: .35rem .6rem; border: 1px solid #ced4da; border-radius: 4px; font-size: .875rem; }
    </style>
</head>
<body>

    <a href="{{ route('admin.orders.index') }}" class="btn btn-secondary btn-sm" style="margin-bottom:1rem;">&larr; All Orders</a>
    ...
    <div class="card" style="margin-bottom:1.25rem;">
        ...
        <div style="max-width:280px;margin-left:auto;margin-top:.75rem;">
            ...
            <div class="summary-row"><span>Shipping ({{ $order->shipping_label }})</span><span>${{ number_format($order->shipping_cost, 2) }}</span></div>
            <div class="summary-row summary-total"><span>Total</span><span>${{ number_format($order->total, 2) }}</span></div>
        </div>
    </div>
