    ...
    <div class="card shadow-sm border-0 rounded-3">
        <div class="card-body">
            ...
            <p class="small text-muted mb-3">
                Drag rows to reorder. The <span class="badge bg-success" style="font-size:.72rem;">thumbnail</span> image is used as the primary product image.
            </p>
                ...
                <form id="reorder-form" method="POST" action="{{ route('admin.products.images.reorder', $product) }}">
                    ...
                    <ul id="image-list" class="list-unstyled">
                        @foreach($product->images as $i => $path)
                            <li class="d-flex align-items-center gap-3 p-2 bg-light border rounded mb-2" data-path="{{ $path }}">
            ...
            @else
                <p class="text-muted">No images uploaded yet. <a href="{{ route('admin.products.edit', $product) }}">Go to Edit</a> to upload images.</p>
            @endif
        </div>
    </div>
</div>
